...
class AppLogin(APIView):
    def post(self,request):
        ...
        user_pw = request.data.get('usr_pw', "")
        ...
        if check_password(user_pw, user.user_pw):
            return Response(dict(msg="로그인 성공", user_id=user.user_id, birth_day=user.birth_day, gender=user.gender, email=user.email, name=user.name, age=user.age))
        else:
